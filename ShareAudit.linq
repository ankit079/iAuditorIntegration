...
class Program
{
	static string auditSharepath = "https://api.safetyculture.io/audits/audit_e7b35a42aa2841ad9baf3184bd902b47/share";	
	static string getAuditIdpath = "https://api.safetyculture.io/audits/search?field=audit_id&field=modified_at";
	static string getUsersPath = "https://api.safetyculture.io/users";
	...
	static void Main()
	{
		...
		var response = P.getUsers(getUsersPath);
		response.Dump();
...
//		eachAuditResult.Dump();		
	...
	async Task<Result> getAuditId(string getAuditIdpath)
	{
		Authentication();
		Result result = null;
		
		var response = await client.GetAsync(getAuditIdpath);
...
//		}						
	...
	async Task<string> getUsers(string path)
	{
		Authentication();
