...
class Program
{
	...
	static void Main()
	{
		...
		var result = P.getEachAuditResult(_apipath);
		result.Dump();
	}
	...
	async Task<AuditData> modifyAudit(excelPreAuditData data, AuditData auditResult)
	{		
		{	var b = auditResult.items.Where(i => i.label == "Site Functional Location").Select(i =>i.responses.text);
			HttpResponseMessage response = await client.PutAsJsonAsync(
				$"https://api.safetyculture.io/audits/{auditResult.audit_id}", auditResult);
			response.EnsureSuccessStatusCode();
